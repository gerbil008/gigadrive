...
beast::ssl_stream<beast::tcp_stream> filestream;

websocket::stream<beast::ssl_stream<beast::tcp_stream>> filews;
...
std::string ca_path = "";
std::vector<websocket::stream<beast::ssl_stream<beast::tcp_stream>>> active_conns;
...
void handle_filesession(tcp::socket socket, net::ssl::context& ctx) {
    try {
        beast::ssl_stream<beast::tcp_stream> filestream(std::move(socket), ctx);

        filestream.handshake(net::ssl::stream_base::server);
        websocket::stream<beast::ssl_stream<beast::tcp_stream>> filews(std::move(filestream));
        filews.accept();
